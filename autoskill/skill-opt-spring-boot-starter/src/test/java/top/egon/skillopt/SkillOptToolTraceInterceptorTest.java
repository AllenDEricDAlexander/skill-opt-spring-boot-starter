...
class SkillOptToolTraceInterceptorTest {
    ...
    void recordsToolCallRequestAndResponse() {
        ...
        ToolCallRequest request = new ToolCallRequest("read_skill", "{\"skill_name\":\"poem-intent\"}", "call-1",
                null);

        ToolCallResponse response = interceptor.interceptToolCall(request,
                current -> ToolCallResponse.of(current.getToolName(), current.getToolCallId(), "skill content"));
