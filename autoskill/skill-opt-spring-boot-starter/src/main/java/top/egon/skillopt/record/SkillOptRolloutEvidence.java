 ...
 */
public record SkillOptRolloutEvidence(String caseId, String input, Path skillFile, String skillHash,
    String output, double score, boolean passed, List<SkillOptToolCall> toolCalls, String error,
    long durationMillis) {
   ...
   */
  public static SkillOptRolloutEvidence success(String caseId, String input, Path skillFile,
      String output, double score, List<SkillOptToolCall> toolCalls) {
    return new SkillOptRolloutEvidence(caseId, input, skillFile, SkillOptHash.sha256(skillFile),
        output, score, score > 0.0, toolCalls, "", 0L);
  }

  /**
   * Creates failed rollout evidence while keeping the flow traceable.
   */
  public static SkillOptRolloutEvidence failure(String caseId, String input, Path skillFile,
      Exception ex, long durationMillis) {
    String error = ex == null ? "" : ex.getClass().getSimpleName() + ": " + ex.getMessage();
    return new SkillOptRolloutEvidence(caseId, input, skillFile, SkillOptHash.sha256(skillFile), "",
        0.0, false, List.of(), error, durationMillis);
  }
  ...
  public SkillOptRolloutEvidence withDurationMillis(long durationMillis) {
    return new SkillOptRolloutEvidence(caseId, input, skillFile, skillHash, output, score, passed,
        toolCalls, error, durationMillis);
  }
}
