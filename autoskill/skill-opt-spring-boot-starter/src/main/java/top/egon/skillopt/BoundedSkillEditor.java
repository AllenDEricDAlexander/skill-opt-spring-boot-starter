...
public class BoundedSkillEditor {
    ...
    private String add(String skillContent, SkillEditOperation edit) {
        ...
        return skillContent.substring(0, insertIndex) + edit.content() + skillContent.substring(insertIndex);
    }
    ...
    private String delete(String skillContent, SkillEditOperation edit) {
        ...
        return skillContent.substring(0, index) + skillContent.substring(index + edit.target().length());
    }
    ...
    private String replace(String skillContent, SkillEditOperation edit) {
        ...
        return skillContent.substring(0, index) + edit.content() + skillContent.substring(index + edit.target().length());
    }

    private void requireTarget(SkillEditOperation edit) {
        if (edit.target().isBlank()) {
            throw new IllegalArgumentException(edit.type().name().toLowerCase() + " target must not be blank");
        }
    }
